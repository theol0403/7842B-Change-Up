...
void middleRedAuton() {
  ...
  Robot::odom()->setState({1_ft, 5_ft, 90_deg});
  ...
  // allign with stack
  chassis.strafeToPoint(
    {4_ft, 5_ft}, OdomController::makeAngleCalculator(0_deg), 1,
  ...
  chassis.strafeToPoint({2.5_ft, 7.5_ft}, OdomController::makeAngleCalculator(0_deg), 1);

  // push cube
  chassis.strafeToPoint({3_ft, 9.5_ft}, OdomController::makeAngleCalculator(0_deg), 1);

  // get different angle on cube
  chassis.strafeToPoint({4_ft, 10_ft}, OdomController::makeAngleCalculator(-45_deg), 1);
