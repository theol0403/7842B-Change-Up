...
void autonomous() {
  ...
  // back up
  asyncTask(roll(out); sleep(200); roll(off); sleep(500); roll(out););
  ...
  asyncTask(sleep(100); roll(outSlow););

  // back up
  drive(-0.6_ft);
  asyncTask(sleep(200); roll(forcePoop); sleep(400); roll(loadingPoop););
  turn(186_deg);
  ...
  asyncTask(sleep(500); roll(loadingPoop););
  ...
  asyncTask(sleep(150); roll(out); sleep(200); roll(loading););
  drive(-0.1_ft);
  Robot::model()->stop();
  ...
  asyncTask(sleep(400); roll(forceShoot); sleep(300); roll(loading););
  move(Line({2.4_ft, -2.8_ft}), {.start = 180_deg});

  asyncTask(sleep(300); roll(loadingPoop););
  turn(10_deg);
  ...
  drive(-0.1_ft);
  shootCorner();
  asyncTask(sleep(100); roll(out); sleep(200); roll(off););
